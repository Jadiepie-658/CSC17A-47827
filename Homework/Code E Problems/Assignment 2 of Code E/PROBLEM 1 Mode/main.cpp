...
int *mode(const int *array,int arySize){
    ...
    {//if first position of array is equal to i
        if(array[i - 1] == i)
        {
           ary[i] = modeAry[0];
    ...
    for(int i = 0; i < arySize; i++)
    {
        ...
        //assigning the modes to the array
        if(count == repeat)
        {
            modeAry[i + 1] = i;
        }
        if(repeat == i)
        {
            modeAry[i] = i; //num of modes
        }
        count = 0;
    }
    
    //Allocate the mode array
   modeAry[0];//Stub returns no modes
    modeAry[1];//Stub returns Frequency 1
